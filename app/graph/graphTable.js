...
Ext.define('openHAB.graph.graphHighcharts', {
    ...
    initComponent:function () {
        ...
        function addGraphData(channel, json) {
			...
			// Mark that we'e received this channel
			rawData[channel].received = true;

			// If we have data, process it into the right format for highcharts
			// This probabl ought to run in parallel with the next channel being received
			if(json != null) {
				...
				rawData[channel].data = newSeries;
            }
...
//                    }
            ...
            options.series[s].data = rawData[chan].data;
            // TODO: Use the item label, not name
            options.series[s].name = rawData[chan].name;
            options.series[s].yAxis = rawData[chan].yAxis;
...
//                    if (dmDev == null)
        ...
        function updateChart(channels, start, stop) {
            ...
            for (var chan = 0; chan < channels.length; chan++) {
				...
				// TODO: Resolve teh yaxis properly
