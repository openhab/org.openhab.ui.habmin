angular.module('HABmin.chart', [
  ...
  function DashboardChartCtrl($scope, $modal, locale, PersistenceItemModel, PersistenceServiceModel, PersistenceDataModel, ChartListModel, ChartSave, growl, VisDataSet, $interval, $timeout) {
    ...
    $scope.items = [];
    $scope.services = [];
    PersistenceItemModel.get().then(function (items) {
      $scope.items = items;
      if ($scope.items != null) {
    ...
    ChartListModel.getList().then(function (charts) {
      $scope.charts = charts;
    ...
    PersistenceServiceModel.query().$promise.then(function (data) {
      ...
      if (duration.asMilliseconds() !== 0) {
        ...
        refreshTimer = $interval(function () {
          ...
          dataItems.add({
            x: now,
            y: count,
            group: 'Outside_RainGauge_Counter'
          });
        }, 2000);
      } else {
        $scope.refreshPeriod = '0';
    ...
    $scope.onRangeChange = function (period) {
      ...
      if (p.s.year == p.e.year) {
        $scope.graphTimeline = p.s.day.name + ' ' + p.s.day.number + '-' + p.s.month.name + '  -  ' + p.e.day.name + ' ' + p.e.day.number + '-' + p.e.month.name + ' ' + p.s.year;
        if (p.s.month.number == p.e.month.number) {
          $scope.graphTimeline = p.s.day.name + ' ' + p.s.day.number + '  -  ' + p.e.day.name + ' ' + p.e.day.number + ' ' + p.s.month.name + ' ' + p.s.year;
          if (p.s.day.number == p.e.day.number) {
            ...
            $scope.graphTimeline = p.s.hour + ':' + p.s.minute + '  -  ' + p.e.hour + ':' + p.e.minute + ' ' + p.s.day.name + ' ' + p.s.day.number + ' ' + p.s.month.name + ' ' + p.s.year;
          }
        }
      } else {
        $scope.graphTimeline = p.s.day.name + ' ' + p.s.day.number + '-' + p.s.month.name + ', ' + p.s.year + '  -  ' + p.e.day.name + ' ' + p.e.day.number + '-' + p.e.month.name + ', ' + p.e.year;
      }
      if (!$scope.$$phase) {
        $timeout(function () {
          $scope.$apply();
    ...
    function _displayChart(id) {
      ChartListModel.getChart(id).then(function (chart) {
    ...
    function _loadItem(itemRef, start, stop) {
      ...
      var me = this;
      PersistenceDataModel.get($scope.selectedService, itemRef, start, stop).then(function (response) {
    ...
    function addSeries(curData, newData, repeatTime, group) {
      ...
      for (var cntNew = 0; cntNew < newData.length; cntNew++) {
        ...
        if (newTime > lastTime + repeatTime) {
          curData.push({
            x: newTime - repeatTime,
            y: Number(newData[cntNew].state),
            group: group
          });
        }
        lastTime = newTime;
    ...
    return function ($scope, element) {
      ...
      $scope.$watch($scope.getWindowDimensions, function (newValue, oldValue) {
        ...
        $scope.styleItemList = function () {
          return { 'height': newValue.h - 232 + 'px' };
        };
        $scope.styleChartList = function () {
          return { 'height': newValue.h - 165 + 'px' };
        };
        $scope.styleChartPanel = function () {
          return { 'height': newValue.h - 93 + 'px' };
        };
        $scope.styleChart = function () {
          return { 'height': newValue.h - 132 + 'px' };
        };
      }, true);
