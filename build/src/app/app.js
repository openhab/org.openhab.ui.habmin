angular.module('HABmin', [
  ...
  delimiter: '::'
}).value('localeSupported', ['en-GB']).value('localeFallbacks', { 'en': 'en-GB' }).config([
  '$stateProvider',
  '$urlRouterProvider',
  'growlProvider',
  'pickAColorProvider',
  'ngBlocklyProvider',
  function myAppConfig($stateProvider, $urlRouterProvider, growlProvider, pickAColorProvider, ngBlocklyProvider) {
    ...
    pickAColorProvider.setOptions({ inlineDropdown: true });
  ...
  function HABminCtrl($scope, $location, SitemapModel, growl, UserService, UserChartPrefs, UserGeneralPrefs, BindingModel) {
    ...
    $scope.sitemaps = null;
    SitemapModel.getList().then(function (data) {
      $scope.sitemaps = data;
    }, function (reason) {
      growl.warning(locale.getString('habmin.mainErrorLoadingSitemaps'));
    });
    BindingModel.getList().then(function (data) {
      var bindings = [];
