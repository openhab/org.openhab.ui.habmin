  ...
  function ZwaveBindingCtrl($scope, locale, growl, $timeout, $window, $http, timeAgo, $interval) {
    ...
    $scope.stateOnline = function (node) {
      ...
      return locale.getString('zwave.zwaveStage', [
        locale.getString('zwave.zwaveStage' + node.nodeStage),
        lastTime,
        status
      ]);
    };
    ...
    $scope.stateHeal = function (node) {
      ...
      t = locale.getString('zwave.' + state, [
        t,
        node.healStage,
        node.healFailStage
      ]);
    ...
    $scope.zwaveAction = function (domain, action) {
      $http.put(url + 'action/' + domain, action).success(function (data) {
    ...
    $scope.updateNodes = function () {
      $http.get(url + 'nodes/').success(function (data) {
        var newList = {};
        var count = 0;
    ...
    function updateStatus(id) {
      $http.get(url + 'nodes/' + id + '/status/').success(function (data) {
    ...
    function updateInfo(id) {
      $http.get(url + 'nodes/' + id + '/info/').success(function (data) {
        if (data.records === undefined) {
          return;
        }
        if (data.records[0] === undefined) {
    ...
    function updateConfig(id) {
      $http.get(url + 'nodes/' + id + '/parameters/').success(function (data) {
    ...
    function updateAssociations(id) {
      $http.get(url + 'nodes/' + id + '/associations/').success(function (data) {
        if (data.records === undefined || data.records.length === 0) {
    ...
    function updateAssociationGroup(domain) {
      $http.get(url + domain).success(function (data) {
        if (data.records === undefined || data.records.length === 0) {
        } else {
    ...
    function updateNeighbors(id) {
      $http.get(url + 'nodes/' + id + '/neighbors/').success(function (data) {
        if (data.records === undefined || data.records.length === 0) {
          return;
        }
        var domain = data.records[0].domain.split('/');
    ...
    function createNetworkMap(root) {
      ...
      console.log('Setting network data', angular.toJson({
        nodes: nodes,
        edges: edges
      }));
      $scope.networkNodes = {
    ...
    return function ($scope, element) {
      ...
      $scope.$watch($scope.getWindowDimensions, function (newValue, oldValue) {
        ...
        $scope.styleList = function () {
          return { 'height': newValue.h - 161 + 'px' };
        };
      }, true);
