ace.define('ace/mode/openhabrules', ['require', 'exports', 'module', 'ace/lib/oop', 'ace/mode/text', 'ace/tokenizer',
    'ace/mode/openhabrules_highlight_rules', 'ace/mode/matching_brace_outdent', 'ace/range', 'ace/worker/worker_client',
    'ace/mode/behaviour/cstyle', 'ace/mode/folding/cstyle'], function (require, exports, module) {
...
ace.define('ace/mode/openhabrules_highlight_rules',
    ['require', 'exports', 'module', 'ace/lib/oop', 'ace/mode/doc_comment_highlight_rules',
        ...
        var openHABRulesHighlightRules = function () {
            var keywordMapper = this.createKeywordMapper({
                ...
                "support.function": "rule|when|then|end",//            "alert",
            ...
            this.add("braces", "insertion", function (state, action, editor, session, text) {
                ...
                if (text == '{') {
                    ...
                    if (rightChar == '}' || closing !== "") {
                        ...
                        var indent = this.getNextLineIndent(state, line.substring(0, cursor.column),
            ...
            this.add("string_dquotes", "insertion", function (state, action, editor, session, text) {
                if (text == '"' || text == "'") {
                    ...
                    if (selected !== "" && selected !== "'" && selected != '"' && editor.getWrapBehavioursEnabled()) {
                        ...
                        if (!token || (quotepos < 0 && token.type !== "comment" && (token.type !== "string" ||
                            ((selection.start.column !== token.value.length + col - 1) &&
                            token.value.lastIndexOf(quote) === token.value.length - 1)))) {
        ...
        (function () {
            ...
            this.getFoldWidgetRange = function (session, foldStyle, row, forceMultiline) {
                ...
                if (match) {
                    ...
                    if (match[1]) {
                        return this.openingBracketBlock(session, match[1], row, i1);
                    }

                    var range = session.getCommentFoldRange(row, i1 + match[0].length, 1);
                ...
                if (match) {
                    var i2 = match.index + match[0].length;

                    if (match[1]) {
                        return this.closingBracketBlock(session, match[1], row, i2);
                    }

                    return session.getCommentFoldRange(row, i2, -1);
                }
            };
