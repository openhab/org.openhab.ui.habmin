 ...
 */
angular.module('sitemapColorWidget', [
    'HABmin.iconModel',
    'toggle-switch',
    'rzModule'
])
    .directive('sitemapColor', function ($interval, ImgFactory) {
        return {
            restrict: 'E',
            ...
            '    <small>' +
            '      <toggle-switch ng-showx="showSwitch" ng-model="switchValue" on-label="ON" off-label="OFF"></toggle-switch>' +
            '    </small>' +
            '  </span>' +
            ...
            link: function ($scope, element, attrs, controller) {
                ...
                if ($scope.widget.item !== undefined) {
                    ...
                    $scope.$watch('colorValue', function (newValue, oldValue) {
                            console.log("COLOR: Changed slider", $scope.widget.label, newValue, oldValue);
                            if (newValue != $scope.currentColorValue) {
                                // Keep a record of the current value so we can detect changes from the GUI
                                // and avoid changes coming from the server!
                                $scope.currentColorValue = newValue;

                                if (!angular.isDefined(timer)) {
                                    // Send an initial update, then after this we do it from the timer!
                                    $scope.$emit('habminGUIUpdate', $scope.widget.item.name,
                                        $scope.currentColorValue + ",100,100");
                                    var latestSliderValue = $scope.currentColorValue;
                                    timer = $interval(function () {
                                            if ($scope.currentColorValue != latestSliderValue) {
                                                latestSliderValue = $scope.currentColorValue;
                                                $scope.$emit('habminGUIUpdate', $scope.widget.item.name,
                                                    $scope.currentColorValue + ",100,100");
                                            }
                                            else {
